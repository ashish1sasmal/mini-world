...
def upload_file(source_path, dest_path, bucket_name='letsmeet'):
...
# get all Files
def get_all_files_p(parent_folder,folder_name, bucket_name='letsmeet'):
...
def upload_file_obj(file, dest_path="", bucket_name='letsmeet'):
    print("inside upload_file_obj")
    try:
        print(f"Dest Path {dest_path}")
        result = s3.Bucket(bucket_name).put_object(Key=dest_path, Body=file)
        print(('result ', result))
        print('upload_file success')
        return 1
    except Exception as e:
...
def move_file(from_path,dest_path, bucket_name='letsmeet'):
    ...
    try:
        ...
        r = s3.meta.client.copy(copy_source, 'letsmeet', dest_path,ExtraArgs={"ACL":'public-read'})
...
def copy_file(from_path,dest_path, bucket_name='letsmeet'):
...
def delete_file(file_path, bucket_name='letsmeet'):
...
def download_file(file_path, bucket_name = 'letsmeet'):
    try:
